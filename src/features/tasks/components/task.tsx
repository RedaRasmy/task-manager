...
import { format, isToday as isTodayFunction } from "date-fns"
...
export default function Task({ task }: { task: TaskType }) {
    ...
    const isToday = (!!task.date && isTodayFunction(task.date))
    ...
    return (
        <div
            className={cn("bg-gray-100 px-4 py-2 grid grid-cols-[30px_1fr_auto] items-center my-1 rounded-md cursor-pointer", {
                "bg-priority-high-background": task.priority === "high",
                "bg-priority-medium-background": task.priority === "medium",
        ...
        >
            ...
            <div className="flex items-center gap-2">
                { task.date && <p className="text-xs text-nowrap">{isToday ? "Today" : format(task.date, "LLLL d")}</p>}
